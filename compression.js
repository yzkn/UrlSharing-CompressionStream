// https://github.com/stinbox/lz-string-vs-compression-stream
...
export async function decompressFromBase64(input) {
...
export async function compressToUTF16(input) {
  const upstream = createUpstream(textEncoder.encode(input));
  const compression = new CompressionStream("deflate");
  const stream = upstream.pipeThrough(compression);
  const compressed = await new Response(stream).arrayBuffer();
...
export async function decompressFromUTF16(input) {
...
export async function decompressFromUint8Array(input) {
...
export async function compressToEncodedURIComponent(input) {
  const withBase64 = await compressToBase64(input);
  return withBase64.replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
}

export async function decompressFromEncodedURIComponent(input) {
